...
DOCKER_ORG=${DOCKER_ORG:-"unieai"}
RELEASE_IMAGE_REPO=${RELEASE_IMAGE_REPO:-"${DOCKER_ORG}/release"}
FINAL_IMAGE_REPO=${FINAL_IMAGE_REPO:-"${DOCKER_ORG}/unieai"}
UNIEAI_COMMON_BUILD_ARGS="--build-arg=VERSION \
    --build-arg=GOFLAGS \
    --build-arg=UNIEAI_CUSTOM_CPU_DEFS \
    --build-arg=UNIEAI_SKIP_CUDA_GENERATE \
    --build-arg=UNIEAI_SKIP_CUDA_11_GENERATE \
    --build-arg=UNIEAI_SKIP_CUDA_12_GENERATE \
    --build-arg=CUDA_V11_ARCHITECTURES \
    --build-arg=CUDA_V12_ARCHITECTURES \
    --build-arg=UNIEAI_SKIP_ROCM_GENERATE \
    --build-arg=UNIEAI_FAST_BUILD \
    --build-arg=CUSTOM_CPU_FLAGS \
    --build-arg=GPU_RUNNER_CPU_FLAGS \
    --build-arg=AMDGPU_TARGETS"

echo "Building Unieai"
