...
var (
	...
	CLIName    = "unieai"
	AppDir     = "/opt/Unieai"
	AppDataDir = "/opt/Unieai"
	// TODO - should there be a distinct log dir?
	UpdateStageDir   = "/tmp"
	AppLogFile       = "/tmp/unieai_app.log"
	ServerLogFile    = "/tmp/unieai.log"
	UpgradeLogFile   = "/tmp/unieai_update.log"
	Installer        = "UnieaiSetup.exe"
	LogRotationCount = 5
)
...
func init() {
	if runtime.GOOS == "windows" {
		...
		AppDataDir = filepath.Join(localAppData, "Unieai")
		...
		if errors.Is(err, os.ErrNotExist) {
			if err := os.MkdirAll(AppDataDir, 0o755); err != nil {
				slog.Error(fmt.Sprintf("create unieai dir %s: %v", AppDataDir, err))
			}
		}
