...
func start(ctx context.Context, command string) (*exec.Cmd, error) {
	...
	if err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			return nil, fmt.Errorf("stat unieai server log dir %s: %v", logDir, err)
		}

		if err := os.MkdirAll(logDir, 0o755); err != nil {
			return nil, fmt.Errorf("create unieai server log dir %s: %v", logDir, err)
		}
	}
	...
	if cmd.Process != nil {
		slog.Info(fmt.Sprintf("started unieai server with pid %d", cmd.Process.Pid))
	}
	slog.Info(fmt.Sprintf("unieai server logs %s", ServerLogFile))
